...
export default function UpdateTugasPkl() {
    ...
    // useEffect(() => {
    //     if (!!data === true) {
    //         // const [year, month, day] = data.batas_waktu.split('-')
    //         setInitialState({ ...data, batas_waktu: data.batas_waktu })
    //         // setInitialState({ ...data, batas_waktu: { year, month, day } })
    //     }
    // }, [data])
    useEffect(() => {
        if (data) {
            setInitialState({
                ...data, batas_waktu: data.batas_waktu
            });
        }
    }, [data]);
    ...
    const onSubmit = async (values) => {
        ...
        try {
            const formattedValues = {
                ...values,
                batas_waktu: values.batas_waktu, // Pastikan ini sudah terformat dengan benar dari BatasWaktuPicker
            };
            const response = await updateTugasPkl(id, formattedValues);
                            ...
                            <div className="grid grid-cols-3 gap-y-3 gap-x-5 shadow-md p-5">
                                ...
                                <section className="col-span-3 lg:col-span-1">
                                    ...
                                    <BatasWaktuPicker
                                        name="batas_waktu"
                                        value={values.batas_waktu}
                                        onChange={date => setFieldValue("batas_waktu", date)} // pastikan mengirimkan string
                                    />
                                    {touched.batas_waktu && errors.batas_waktu && (
                                        <div className="ui pointing red basic label ">
                                            {errors.batas_waktu}
