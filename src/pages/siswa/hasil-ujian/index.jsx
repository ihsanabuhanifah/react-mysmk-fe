...
import { LoadingPage, PaginationTable, TableLoading } from '../../../components'
		...
		<LayoutSiswa title="Hasil Ujain">
			<div className="h-full pl-2 pr-5 w-full overflow-y-auto">
				<Table className="ui celled table structured">
					...
					<Table.Body>
						<TableLoading count={5} isLoading={isFetching} data={data?.data} messageEmpty="Data tidak ditemukan">
              {data?.data.map((value, i) => (
                <Table.Row key={i}>
                  ...
                  <Table.Cell>{value.exam.replace(/[\[\]]/g, '').replace(/,\s*/g, ' - ')}</Table.Cell>
