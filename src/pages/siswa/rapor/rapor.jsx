...
import { Button, Icon, Input, Menu, Sidebar, Table, TableRow } from 'semantic-ui-react'
...
export default function Rapor() {
	...
	const { data, isFetching, setParams, dataTa, params, dataKelas } = useGetHasilBelajar()
	let [visible, setVisible] = React.useState(false)

	return (
		<LayoutSiswa title="Rapor">
			<Sidebar as={Menu} animation="overlay" icon="labeled" inverted direction="right" onHide={() => setVisible(false)} vertical visible={visible} width="wide">
				<FilterHasilBelajar params={params} setParams={setParams} setVisible={setVisible} dataTa={dataTa} dataKelas={dataKelas} />
			</Sidebar>
				...
				<Table className='ui celled table structured'>
					<Table.Header>
						<TableRow>
							...
							<Table.HeaderCell colspan={5}>Rata-Rata Nilai</Table.HeaderCell>
					...
					<Table.Body>
						<TableLoading count={13} isLoading={isFetching} data={data?.data} messageEmpty="Data tidak ditemukan">
							{data?.data.map((value, i) => (
								<Table.Row key={i}>
									...
									<Table.Cell>{format(parseISO(value?.createdAt), 'yyyy-MM-dd')}</Table.Cell>
									<Table.Cell>
										<Button onClick={() => navigate(`/siswa/rapor/${value.mapel.id}/${value?.tahun_ajaran?.id}`)} content="Detail" type="button" size="medium" color="green" />
									</Table.Cell>
								</Table.Row>
							))}
						</TableLoading>
