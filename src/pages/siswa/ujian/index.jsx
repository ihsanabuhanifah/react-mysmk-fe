...
import { Input,Button, Dropdown, Icon, Loader, Menu, Sidebar } from 'semantic-ui-react'
...
export default function UjianSiswa() {
	...
	const { data, setParams, isFetching, dataMapel, params, loadMapel } = useExam(examActive)
	...
	return (
		<LayoutSiswa title="Exam">
			<Sidebar as={Menu} animation="overlay" icon="labeled" inverted direction="right" onHide={() => setVisible(false)} vertical visible={visible} width="wide">
				<FilterUjian params={params} setParams={setParams} setVisible={setVisible} dataMapel={dataMapel} />
			</Sidebar>
								...
								handleExam={() => {
									if (item.ujian.is_hirarki) {
										...
										if (item.ujian.urutan !== 1) {
											const currentExam = newArr.find((exam) => exam.ujian.urutan === item.ujian.urutan - 1)
											if (currentExam.status === 'finish') {
												setExamActive(item.id)
											} else {
												customToast('Selesaikan yang sebelumnya terlebih dahulu!')
											}
										} else {
											setExamActive(item.id)
										}
